...
import { MapContainer as Map, Marker, TileLayer, Popup, Tooltip, CircleMarker, LayersControl, FeatureGroup } from 'react-leaflet'
...
const MapComponent = (props) => {
	...
	useEffect(() => {
		...
		fetchData(props.match.params.data, props.match.params.gsheetID).then((data) => {
			if (data.length <= 0) {
	...
	const { circleSize, fetchedData, lat1, lng1, lat2, lng2, googleData, dataSource, lastUpdate } = mapData
	...
	let GoogleMarkers = googleData
		...
		.map((el, i) => (
			<Marker key={`marker-${i}`} position={[parseFloat(el.lat), parseFloat(el.lng)]} icon={mapData.dataSource ? greenMarkerIcon : orangeMarkerIcon}>
				{!isMobile && (
					<Tooltip>
						<p>
				...
				<Popup>
					{!isMobile && el.imageUrl && (
						<a href={el.articleLink} target={'_blank'} rel={'noopener noreferrer'}>
							<img src={el.imageUrl} width={'100%'} alt={el.title} />
						</a>
					)}
					<div className={'flow popup-textbox'}>
						<h3>{el.title}</h3>
						<div className={`description-wrapper ${el.imageUrl ? 'hidden' : ''}`}>{renderHTML(el.description)}</div>
						{el.articleLink && (
		...
		CircleMarkers = fetchedData
			...
			.map((el, i) => (
				<CircleMarker
					...
					fillColor={'#a03c50'}
					stroke={false}
					radius={circleSize[0] + (parseInt(el.Confirmed) / maxAmount) * circleSize[1]}
				>
					{!isMobile && (
						<Tooltip>
	...
	return (
		<StyledDiv id={'map-component'} data-iframe-height>
			<Map bounds={bounds} style={{ width: '100%', height: '100vh' }} tap={isMobile}>
