...
// Import renderHTML to render HTML
import renderHTML from 'react-render-html'

function PaperSnippets(props) {
	...
	useEffect(() => {
		console.log('Fetch with papaparse')
	...
	const relevantData = group === 'all' ? data : data.filter((el) => el.group === group)
	...
	function CloseIcon() {
		return (
			<svg xmlns='http://www.w3.org/2000/svg' className={'fill-current inline w-10 h-10'} viewBox='0 0 100 100'>
				<path
					fillRule='evenodd'
	...
	const OverlayContent = ({ data }) => {
		return (
			<div className={'flex flex-col justify-center items-start w-full h-full absolute top-0 left-0 cursor-pointer'}>
				<h1 className={'text-5xl mb-8'}>{data.full_name}</h1>
				<div className={'text-3xl'}>{renderHTML(data.content)}</div>
				<motion.p
					className={'absolute top-0 right-0 text-red-400 hover:bg-red-400 hover:text-white px-4 py-4 rounded-full'}
					whileHover={{
						scale: 1.1,
					}}
				>
	...
	return (
		<div data-iframe-height={true}>
			{relevantData ? (
				<div className={`relative grid grid-cols-${mobileCols} sm:grid-cols-${desktopCols} w-4/5 mx-auto my-8`}>
					{relevantData.map((el, i) => {
						return <Bubble key={`bubble-wrapper-${i}`} data={el} i={i} clickHandler={clickHandler} currentActive={currentActive} />
					})}
					{currentActive >= 0 && (
						<div
							className={'m-4 w-full h-full absolute top-0 left-0 bg-white bg-opacity-95 flex justify-center items-center'}
