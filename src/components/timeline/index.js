...
const TimelineComponent = (props) => {
	...
	useEffect(() => {
		Papa.parse(`https://docs.google.com/spreadsheets/d/${props.match.params.src}/pub?output=csv`, {
			download: true,
			header: true,
			complete: ({ data }) => {
				setDataFunc(data)
	...
	const mappedEvents = _.map(allData, (event, index) => {
		return (
			<div className={`year-container single-event column ${index}`}>
				...
				<span className={'line'} />
				<SingleEventComponent data={event} visible={event.visible} collapsed={collapsed} />
			</div>
		)
	})
	...
	const mappedTitle = _.map(allData, (event, index) => {
		...
		return (
			<div className={'line-left'}>
				{currentYear !== '' && <h3 className={`title-centered ${titleClassName}`}>{currentYear}</h3>}
				<SingleEventComponent data={event} visible={event.visible} collapsed={collapsed} type={'by-title'} />
			</div>
		)
	})
	...
	return (
		...
		>
			...
			{!data.isLoading && !options && (
				<div>
					<div className={'timeline-container ui two column grid'}>{mappedYears}</div>
				</div>
			)}
			{!data.isLoading && options === 'by-year' && (
				<div>
					<div className={'timeline-container ui two column grid by-year'}>{mappedYears}</div>
				</div>
			)}
			{!data.isLoading && options === 'by-tags' && (
				<div>
					<div className={'timeline-container ui two column grid by-tags'}>{mappedYearsFiltered}</div>
				</div>
			)}
			{!data.isLoading && options === 'by-event' && (
				<div>
					<div className={'timeline-container ui two column grid'}>{mappedEvents}</div>
				</div>
			)}
			{!data.isLoading && options === 'by-title' && (
				<div>
					<div className={'timeline-container ui one column grid by-title'}>{mappedTitle}</div>
				</div>
			)}
		</div>
	)
